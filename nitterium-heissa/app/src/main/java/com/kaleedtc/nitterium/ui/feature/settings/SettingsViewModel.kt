...
class SettingsViewModel(
    ...
    private val availableInstances = listOf(
        application.getString(R.string.nitter_heissa_de_url),
        ...
        when (event) {
            ...
            is SettingsEvent.RemoveCustomInstance -> {
                viewModelScope.launch {
                    ...
                    // If the current URL is removed, fallback to nitter.heissa.de
                    if (state.value.instanceUrl == event.url) {
                        preferencesRepository.setInstanceUrl("https://nitter.heissa.de")
                    }
                }
            }
